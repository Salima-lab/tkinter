...
class Paint():
    def __init__(self):
        ...
        self.brush_button = Button(
            self.root, text='Кисть', command=self.use_brush)
        self.brush_button.grid(row=0, column=1)

        self.color_button = Button(
            self.root, text='Цвет', command=self.choose_color)
        self.color_button.grid(row=0, column=2)

        self.eraser_button = Button(
            self.root, text='Ластик', command=self.erase_button)
    ...
    def paint(self, event):
        ...
        if self.old_x and self.old_y:
            self.can.create_line(self.old_x, self.old_y, event.x, event.y,
